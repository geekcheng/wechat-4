...
public class AjaxAuthenticationFilter implements Filter {
	...
	/**
	    ...
	    */
		private boolean isURILogin(String requestURI, HttpServletRequest request) {
			for (String uri : Globals.INHERENT_URIS) {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain filterChain)
		...
		if (null != request && null != request.getAttribute(Globals.FILTERED_REQUEST)) {
			filterChain.doFilter(request, response);
		} else {
			// ②-2 设置过滤标识，防止一次请求多次过滤
			request.setAttribute(Globals.FILTERED_REQUEST, Boolean.TRUE);
			...
			if ((null == sessioninfo || null == sessioninfo.getManagerUser()) && isURILogin(httpRequest.getRequestURI(), httpRequest)) {
